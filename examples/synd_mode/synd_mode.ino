...
The AS7341 device features three modes to perform a spectral measurement. The
integration mode (INT_MODE) can be configured in register 0x70 (CONFIG)

This Script shows the implementation of SYND Mode INT_MODE = 0x03

Spectral measurement is based on Integration with external start and stop sync
signal Integration is controlled via rising/falling edge on pin GPIO and
register EDGE. If the number of edges on pin GPIO is reached, integration time
is stopped. Actual integration time can be read out in register “ITIME”.
...
void loop() {
  ...
  // Function defined to read out channels with with SYND MODE and SMUX
  // configration 1 for pixels - F1-F4, Clear, NIR
  synD_Mode();
...
//<summary>
// Function executes to read out channels with SYND MODE and SMUX configration 1
// for pixels - F1-F4, Clear, NIR the integration time is defined by the number
// of rising/falling edge between start and stop on pin GPIO Signal, which is
// set to register EDGE (0x72)
//<summary>
void synD_Mode() {
  ...
  // CONFIG (0x70) is used to set the INT_MODE (Bit 1:0) to SYND Mpde by writing
  // 0x03
  INT_MODE(0x03);
  ...
  Serial.print("syn SynEdge to ");
  Serial.println(SYND_EDGES);
  ...
  while (!(isDataReady)) {
    ...
    Serial.print("Pulsing low w/ delay ");
    Serial.println(LOW_PULSE_DELAY);
    digitalWrite(GPIO_OUT_PIN, LOW);
    delay(LOW_PULSE_DELAY);
    digitalWrite(GPIO_OUT_PIN, HIGH);
    low_pulse_count += 1;
  }
  Serial.println("Low pulses: ");
  Serial.println(low_pulse_count);
  // analog and digital saturation are read in Status2
  // Digital saturation - Indicates that the maximum counter value has been
  // reached. Maximum counter value depends on integration time set in the ATIME
  // register
...
// over ram_bank
// reg_bank bit(4) is set to '1' for setting the 0x00-0x7f regiater to reg_bank
...
//<summary>
// Number of falling SYNC-edges between start and stop integration in SynD mode
